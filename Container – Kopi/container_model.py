...
def container(x,ui):
    ...
    if (len(x) != 10):
        print('x-vector must have dimension 10!')
        #exit()
    if (len(ui) != 2):
        print('u-vector must have dimension  2!')
        #exit()

    # Normalization variables
    L = 175                            # Length of ship (m)
    U = math.sqrt(x[0]**2 + x[1]**2)   # service speed (m/s)
    #print(f'Speed: {U} -- X: {x[0]} -- Y: {x[1]}')

    # Check service speed
    if U <= 0:
        print('The ship must have speed greater than zero..')
        #exit()

    if x[9] <= 0:
        print('The propeller rpm must be greater than zero..')
        #exit()
    ...
    J     = uP*U/(n*D)
    KT     = 0.527 - 0.455*J
    uR     = uP*epsilon*math.sqrt(1 + 8*kk*KT/(math.pi*J**2))
    alphaR = delta + math.atan2(vR,uR)
    FN     = - ((6.13*Delta)/(Delta + 2.25))*(AR/(L**2))*(uR**2 + vR**2)*math.sin(alphaR)
    T      = (2*rho*(D**4)/((U**2)*(L**2)*rho))*KT*n*abs(n)

    # Forces and moments
    X    = Xuu*u**2 + (1-t)*T + Xvr*v*r + Xvv*v**2 + Xrr*r**2 + Xphiphi*phi**2 + cRX*FN*math.sin(delta) + (m + my)*v*r

    Y    = Yv*v + Yr*r + Yp*p + Yphi*phi + Yvvv*v**3 + Yrrr*r**3 + Yvvr*v**2*r + Yvrr*v*r**2 + Yvvphi*v**2*phi + Yvphiphi*v*phi**2 + Yrrphi*r**2*phi + Yrphiphi*r*phi**2 + (1 + aH)*FN*math.cos(delta) - (m + mx)*u*r

    K    = Kv*v + Kr*r + Kp*p + Kphi*phi + Kvvv*v**3 + Krrr*r**3 + Kvvr*v**2*r + Kvrr*v*r**2 + Kvvphi*v**2*phi + Kvphiphi*v*phi**2 + Krrphi*r**2*phi +  Krphiphi*r*phi**2 - (1 + aH)*zR*FN*math.cos(delta) + mx*lx*u*r - W*GM*phi

    N    = Nv*v + Nr*r + Np*p + Nphi*phi + Nvvv*v**3 + Nrrr*r**3 + Nvvr*v**2*r + Nvrr*v*r**2 + Nvvphi*v**2*phi + Nvphiphi*v*phi**2 + Nrrphi*r**2*phi + Nrphiphi*r*phi**2 + (xR + aH*xH)*FN*math.cos(delta)

    # Dimensional state derivatives  xdot = [ u v r x y psi p phi delta n ]'
    detM = m22*m33*m44-m32**2*m44-m42**2*m33

    #print(f'X: {X} -- Y: {Y} -- K: {K} -- N: {N}')
    xdot =[X*((U**2)/L)/m11, -((-m33*m44*Y+m32*m44*K+m42*m33*N)/detM)*((U**2)/L), ((-m42*m33*Y+m32*m42*K+N*m22*m33-N*m32**2)/detM)*((U**2)/L**2), (math.cos(psi)*u-math.sin(psi)*math.cos(phi)*v)*U, (math.sin(psi)*u+math.cos(psi)*math.cos(phi)*v)*U, math.cos(phi)*r*(U/L), ((-m32*m44*Y+K*m22*m44-K*m42**2+m32*m42*N)/detM)*((U**2)/L**2), p*(U/L), delta_dot, n_dot]
